<html>
	<head>
		<title>Результаты экзамена по курсу обучения</title>
		...
		<script src="js/jquery-isotopemin.js"></script>
		<script>
	...
	<body>
		...
		<nav class="sideMenu">
			...
			<ul class="mainSections">
				...
				<li>
					<a href="plan.html#examination" id="exam">
						<b>Аттестация</b>
					</a>
				</li>					
			</ul>
		...
		<section class="content" id="detailedResults">
			<article class="description" id="printableArea">
				<h1 class="green">Результаты экзамена<span class="hidden"> по&nbsp;курсу обучения</span></h1>
				<div class="totalResults">
					<?php
					echo "<table width='100%' height='auto' class='studentTotalResults' border='0' cellspacing='0' cellpadding='0'>\n\n";
					$f = fopen("data/total.csv", "r");
				...
				<div class="buttons">
					<a href="data/results.csv" class="btn" target="_blank">Скачать в&nbsp;Excel</a>
				</div>
				<div class="students">
					<h2 class="studentResults">Статистика аттестации</h2>
					<?php
					echo "<table width='100%' height='auto' class='studentResults' border='0' cellspacing='0' cellpadding='0'>\n\n";
					$f = fopen("data/results.csv", "r");
					while (($line = fgetcsv($f)) !== false) {
							echo "<tr>";
							foreach ($line as $cell) {
									echo "<td>" . htmlspecialchars($cell) . "</td>";
					...
					fclose($f);
					echo "\n</table></body></html>";?>
					<input type="button" onclick="printDiv('printableArea')" value="Распечатать ведомость" class="printPage"/>
				</div>
			</article>
		</section>
